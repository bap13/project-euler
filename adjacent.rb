...
verticals = Array.new(20).fill { |i| Array.new(20).fill { |j| grid[j][i] } }

def bt_diagonals(grid)
  grid_copy = grid.map { |i| i.dup }
  shifted_grid = grid_copy.each_index do |i|
    (19-i).times { grid_copy[i].push("0") }
    i.times { grid_copy[i].unshift("0") }
  end
  bt_diagonals = Array.new(39).fill { |i| Array.new(20)
                              .fill { |j| shifted_grid[j][i] } }
end

def tb_diagonals(grid)
  grid_copy = grid.map { |i| i.dup }
  shifted_grid = grid_copy.each_index do |i|
    i.times { grid_copy[i].push("0") }
    (19-i).times { grid_copy[i].unshift("0") }
  end
  tb_diagonals = Array.new(39).fill { |i| Array.new(20)
                              .fill { |j| shifted_grid[j][i] } }
end

mega_array = grid, verticals, bt_diagonals(grid), tb_diagonals(grid)

# Takes in an array of numbers and returns the maximum 
# adjacent product of n numbers
...
mega_array.each do |array|
  array.each do |row|
    tmp_product = max_product(row, 4)
    max_adjacent_product = tmp_product if tmp_product > max_adjacent_product
  end
end
puts max_adjacent_product
